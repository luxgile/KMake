...
{
	return stack->stacktop - stack->stack;
}

void bytestk_push(ByteStack* stack, Byte1 byte)
{
#ifdef DEBUG_SAFETY
	if (bytestk_count(stack) == stack->capacity) printf("Error: Stack Overflow.");
#endif
...
{
#ifdef DEBUG_SAFETY
	if (bytestk_count(stack) + count > stack->capacity) printf("Error: Stack Overflow.");
#endif
