...
void emit_const(KCompiler* compiler, Byte1* bytes, TYPE_ID id)
{
	...
	emit_2bytes(compiler, id, make_const(compiler, bytes, id));
}

void emit_constp(KCompiler* compiler, Byte1* bytes, TYPE_ID id)
{
	emit_byte(compiler, OP_CONSTANT);
	emit_2bytes(compiler, id, make_constp(compiler, bytes, id));
}
...
void read_number(KCompiler* compiler)
{
	...
	emit_const(compiler, &value, TYPEID_DEC);
}
...
void read_unary(KCompiler* compiler)
{
	...
	switch (operatorType)
	{
	case TOKEN_MINUS: emit_byte(&compiler->parser, OP_NEGATE); break;
	case TOKEN_NOT: emit_byte(&compiler->parser, OP_NOT); break;
	default: return;
	}
}
